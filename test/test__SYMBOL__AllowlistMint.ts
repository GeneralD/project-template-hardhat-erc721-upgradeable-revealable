...
describe("Mint __SYMBOL__ as allowlisted member", () => {
    ...
    it("Not allowlisted member's minting is not allowed", async () => {
        ...
        await expect(instance.connect(mike).allowlistMint(5, proof, { value: enoughBadget })).to.revertedWith("invalid merkle proof")
    })
    ...
    it("Allowlisted member can mint but not over the limit", async () => {
        ...
        await expect(instance.connect(jonathan).allowlistMint(quantity, proofOfJonathan, { value: totalPrice })).to.revertedWith("WL minting exceeds the limit")
    ...
    it("Allowlisted member can mint in allowlist mint limit but not over the limit of entire contract", async () => {
        ...
        const proofOfJonathan = tree.getHexProof(keccak256(jonathan.address))
        await expect(instance.connect(jonathan).allowlistMint(quantity, proofOfJonathan, { value: totalPrice })).to.revertedWith("minting exceeds the limit")
    })
    ...
    it("Cannot mint if sent ETH is not enough", async () => {
        ...
        await expect(instance.connect(john).allowlistMint(quantity, proof, { value: paid })).to.revertedWith("not enough eth")
    })
})